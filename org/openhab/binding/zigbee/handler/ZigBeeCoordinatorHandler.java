...
// Dovydas - changed method type deserializeNode from void to boolean
public abstract class ZigBeeCoordinatorHandler extends BaseBridgeHandler implements NodeListener {
    ...
    public boolean deserializeNode(String address) {
        ZigBeeNodeSerializer serializer = new ZigBeeNodeSerializer();

        List<ZigBeeEndpoint> endpoints = serializer.deserializeNode(address);
        if (endpoints == null) {
            logger.warn(" There are no endpoints for deivice {}", address);
            return false;
        }
    ...
    public Object readAttribute(Device device, int clusterId, int attributeIndex) {
        ...
        try {
            ...
            logger.error("Could not get the value for attribute: {} ", attribute);
            e.printStackTrace();
            return null;
        }
    }
    ...
    private class DiscoveryThread extends Thread {
        public void run(ZigBeeNode node) {
            ...
            discoveryService.addThing(node, devices, "ti", "cc2650");
        }
    }
    ...
    public void bindAttribute(String address, int cluster, int attribute) {
        ...
        try {
            ...
            logger.error("{}: Error, could not bind attribute {} for cluster {}", address, attribute, cluster);
            e.printStackTrace();
        }
    }
