...
public class ZigBeeLevelClusterHandler extends ZigBeeClusterHandler implements ReportListener {
    ...
    public List<Channel> getChannels(ThingUID thingUID, Device device) {
        ...
        if (id.endsWith("/2")) {
            label = "Dimmer Kaltweiß";
        }
        if (id.endsWith("/3")) {
            label = "Dimmer Warmweiß";
        }
        if (id.endsWith("/4")) {
            label = "Dimmer Gelb";
        }
        ...
        if (id.endsWith("/7")) {
            label = "Mode -- nicht definiert";
            return channels;
        }

        channels.add(createChannel(device, thingUID, ZigBeeBindingConstants.CHANNEL_SWITCH_DIMMER, "Dimmer", label));
