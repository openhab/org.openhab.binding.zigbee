...
// Dovydas - removed disposeConverter(), implemented handleRefresh() and getAttributes() methods
public class ZigBeeColorClusterHandler extends ZigBeeClusterHandler implements ReportListener {
    ...
    private void getAttributes() {
        try {
            Integer realHue = null;
            Integer realSaturation = null;

            Integer currentHue = (Integer) attrHue.getValue();
            Integer currentSaturation = (Integer) attrSaturation.getValue();

            if (currentHue != null) {
                realHue = currentHue * 360 / 254;
                updateChannelState(new DecimalType(realHue));
            } else {
                logger.error("Can not get current Hue for device {}", address);
            }

            if (currentSaturation != null) {
                realSaturation = currentSaturation / 254;
                updateChannelState(new DecimalType(realSaturation));
            } else {
                logger.error("Can not get current Saturation for device {}", address);
            }

        } catch (ZigBeeClusterException e) {
            logger.error("Error, can not get light attributes for device {}", address);
            e.printStackTrace();
        }
    }
