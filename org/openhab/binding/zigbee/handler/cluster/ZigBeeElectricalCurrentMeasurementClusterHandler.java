...
public class ZigBeeElectricalCurrentMeasurementClusterHandler extends ZigBeeClusterHandler implements ReportListener {
    ...
    private Attribute attrElectricalCurrent;
    ...
    public void initializeConverter() {
        ...
        for (Attribute a : att) {
            if (a.getName().equals(Attributes.DC_CURRENT.getName())) {
                attrElectricalCurrent = a;
            }
        }

        if (attrElectricalCurrent == null) {
        ...
        try {
            Object oValueC = attrElectricalCurrent.getValue();
            int iValueC = (int) oValueC;
            double dValueC = iValueC;
            if (oValueC != null) {
                updateChannelState(new DecimalType(dValueC));
            } else {
    ...
    public void disposeConverter() {
        ...
        if (attrElectricalCurrent != null) {
            coordinator.closeAttribute(attrElectricalCurrent, this);
        }
    ...
    public void receivedReport(String endPointId, short clusterId, Dictionary<Attribute, Object> reports) {
        ...
        if (attrElectricalCurrent != null) {
            Integer value = (Integer) reports.get(attrElectricalCurrent);
    ...
    public List<Channel> getChannels(ThingUID thingUID, Device device) {
        ...
        String labelCurrent = "nicht definiert";
        String id = device.getEndpointId();
        if (id.endsWith("/16")) {
            labelCurrent = "Strom LED in mA";
        }
        if (id.endsWith("/17")) {
            labelCurrent = "Strom System in mA";
        }

        channels.add(createChannel(device, thingUID, ZigBeeBindingConstants.CHANNEL_ELECTRICAL_CURRENT, "Number",
                labelCurrent));
