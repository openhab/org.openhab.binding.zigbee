...
import org.bubblecloud.zigbee.network.packet.zdo.ZDO_BIND_REQ;
import org.bubblecloud.zigbee.network.packet.zdo.ZDO_BIND_RSP;
...
import org.bubblecloud.zigbee.network.packet.zdo.ZDO_UNBIND_REQ;
import org.bubblecloud.zigbee.network.packet.zdo.ZDO_UNBIND_RSP;
...
public class ZigBeeEndpointImpl
     ...
     */
    private static final long CLUSTER_MESSAGE_RESPONSE_TIME_MILLIS = 20000;
    ...
    public ClusterMessage invoke(ClusterMessage input) throws ZigBeeNetworkManagerException {
        synchronized (networkManager) {
            ...
            WaitForClusterResponse waiter = new WaitForClusterResponse(this, transaction, input.getId(),
                    CLUSTER_MESSAGE_RESPONSE_TIME_MILLIS);

            logger.trace("---> SENDING transaction: " + transaction + " TO: " + node.getNetworkAddress() + " with"
                    + " byte 0 " + Integers.getByteAsInteger(node.getNetworkAddress(), 0) + " byte 1 "
            ...
            AF_DATA_CONFIRM response = networkManager
                    .sendAFDataRequest(new AF_DATA_REQUEST(node.getNetworkAddress(), endPointAddress, sender,
                            input.getId(), transaction, (byte) (0) /* options */, (byte) 0 /* radius */, msg));
            ...
            if (response == null) {
                ...
                m_removeAFMessageListener();
                final ResponseStatus responseStatus = ResponseStatus
                        .getStatus(Integers.getByteAsInteger(response.getStatus(), 0));
    ...
    public boolean bindToLocal(int clusterId) throws ZigBeeNetworkManagerException {
        ...
        logger.info("Binding from endpoint {} to {} for cluster {}", new Object[] { getEndpointId(),
                IEEEAddress.toString(networkManager.getIeeeAddress()) + "/" + dstEP, new Integer(clusterId) });
